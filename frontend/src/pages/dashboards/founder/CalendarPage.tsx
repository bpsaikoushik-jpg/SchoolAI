...
export function CalendarPage() {
  ...
  function handleCreate() {
    ...
    createEvent.mutate(
      ...
      { onSuccess: () => { setOpen(false); setTitle(''); setType('meeting'); setStartsAt(toLocalInputValue(new Date())); } }
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setOpen(true)} style={{ background: ACCENT }}>Add event</Button>
      </div>
      <Card>
        {events.length === 0 ? (
          ...
          <div className="space-y-4">
            {events.map((e) => (
              <div key={e.id} className="flex items-center gap-4 border-b border-border-subtle pb-4 last:border-0 last:pb-0">
                ...
                <Badge tone={e.event_type === 'meeting' ? 'warning' : e.event_type === 'class' ? 'info' : 'neutral'}>{e.event_type}</Badge>
              </div>
            ))}
          </div>
        )}
      ...
      <Dialog
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={handleCreate} isLoading={createEvent.isPending} style={{ background: ACCENT }}>Save</Button>
          </div>
        }
      >
        <div className="space-y-4">
          <Input label="Title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Staff meeting" />
